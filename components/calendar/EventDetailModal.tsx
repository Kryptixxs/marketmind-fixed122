...
export function EventDetailModal({ event, onClose }: EventDetailModalProps) {
  // 100% Deterministic, zero-latency rules engine. No AI.
      ...
      <div className="bg-surface border border-border w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col shadow-2xl rounded-sm">
        ...
        <div className="panel-header shrink-0 flex justify-between items-center px-3 py-2 h-auto border-b border-border bg-surface-highlight">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] text-text-tertiary uppercase tracking-widest font-mono">Terminal ID: {event.id} // DETERMINISTIC_RULES_V1.0</span>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <div className="bg-background border border-border p-2 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono font-bold text-text-secondary">N/A</span>
                </div>
                <div className="bg-background border border-border p-2 flex items-center justify-between">
                ...
                <div className="mt-2 p-2 bg-surface-highlight/30 border border-border/50 rounded-sm">
                  <div className="flex items-center gap-2 mb-1">
                    <ShieldAlert size={10} className="text-text-tertiary" />
                    <span className="text-[8px] text-text-tertiary font-bold uppercase">Live Feeds Required</span>
                  </div>
                  <p className="text-[9px] text-text-secondary leading-tight italic">
                    Requires real-time options chain and Level 2 depth data.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-border space-y-3">
              ...
              <div className="bg-accent/5 border border-accent/20 p-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="text-[8px] text-text-tertiary uppercase">Mathematical classification applied</div>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-3 border-t border-border bg-surface-highlight flex justify-between items-center">
          <div className="flex gap-2">
            <button onClick={handleSetAlert} className="px-3 py-1.5 bg-accent text-accent-text text-[10px] font-bold uppercase rounded-sm hover:opacity-90 transition-opacity">
              Set Alert
            </button>
          ...
          <span className="text-[9px] font-mono text-text-tertiary">VANTAGE TERMINAL // CALENDAR_V1 // {new Date().toISOString().split('T')[0]}</span>
        </div>
      </div>
    </div>
  );
