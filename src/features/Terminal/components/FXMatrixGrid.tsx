    ...
    <div className="h-full flex flex-col bg-surface overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {PAIRS.map(pair => {
          ...
          const spread = (Math.random() * 0.5 + 0.2).toFixed(1); // Mock spread
          const vol = (Math.random() * 2 + 4).toFixed(1); // Mock vol
          const carry = (Math.random() * 4 - 2).toFixed(2); // Mock carry
          ...
          return (
            ...
            >
              ...
              <span className="text-right font-mono text-text-tertiary">{spread}</span>
              ...
              <span className="text-right font-mono text-text-tertiary">{vol}%</span>
              <span className={`text-right font-mono ${parseFloat(carry) >= 0 ? 'text-positive/60' : 'text-negative/60'}`}>{carry}%</span>
            </div>
          );
        })}
      </div>
