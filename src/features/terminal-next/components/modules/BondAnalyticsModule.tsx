      ...
      <section className="bg-black min-h-0 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar text-[8px]">
          <div className="grid grid-cols-2 gap-px bg-[#1a1a1a] border-b border-[#1a1a1a]">
            <div className="px-1 py-[2px] bg-[#0a0a0a] text-[#9fb4cd]">BidCum <span className="text-[#4ce0a5] font-bold">{state.orderBook.slice(0, 10).reduce((a, b) => a + b.cumBidSize, 0)}</span></div>
            <div className="px-1 py-[2px] bg-[#0a0a0a] text-[#9fb4cd]">AskCum <span className="text-[#ff7ca3] font-bold">{state.orderBook.slice(0, 10).reduce((a, b) => a + b.cumAskSize, 0)}</span></div>
          </div>
