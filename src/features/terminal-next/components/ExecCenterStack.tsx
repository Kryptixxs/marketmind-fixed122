...
export function ExecCenterStack({ execMode }: { execMode: 'PRIMARY' | 'MICROSTRUCTURE' | 'FACTORS' | 'EVENTS' | 'ESC' }) {
  ...
  const microRows = withMinimumRows(state.orderBook.flatMap((lvl, idx) => [
  ...
  const systemRows = withMinimumRows([...state.systemFeed, ...state.headlines, ...state.alerts].map((line, idx) => ({
    label: `SYS ${idx + 1}`,
    value: line,
  ...
  const blocks: StackBlock[] = [
    { id: 'price', title: `PRICE / MARKET [${execMode}]`, rows: withMinimumRows((depth?.market.indices ?? []).map((r) => ({ label: r.symbol, value: `${r.level.toFixed(2)} (${r.movePct >= 0 ? '+' : ''}${r.movePct.toFixed(2)}%)`, tone: r.movePct >= 0 ? 'positive' : 'negative' })), 16, 'PRICE'), provenance: p.financial },
    { id: 'vol', title: 'VOLATILITY & SKEW', rows: withMinimumRows((depth?.options.surface ?? []).map((r) => ({ label: r.delta, value: `W1 ${r.w1} M1 ${r.m1} M3 ${r.m3} M6 ${r.m6}`, tone: 'accent' })), 12, 'VOL'), provenance: p.flow },
    { id: 'flow', title: 'FLOW & POSITIONING', rows: withMinimumRows((depth?.market.flows ?? []).map((r) => ({ label: `${r.vehicle} ${r.direction}`, value: `${r.flowUsdM >= 0 ? '+' : ''}${r.flowUsdM}M`, tone: r.direction === 'Inflow' ? 'positive' : 'negative' })), 12, 'FLOW'), provenance: p.flow },
    { id: 'peer', title: 'PEER COMPARISON', rows: withMinimumRows((depth?.relationships?.entities ?? []).slice(1).map((e) => ({ label: `${e.symbol} ${e.country}`, value: e.sector, tone: 'neutral' })), 12, 'PEER'), provenance: p.peers },
    ...
    { id: 'events', title: 'EVENT TIMELINE', rows: withMinimumRows((depth?.news.impacts ?? []).map((e) => ({ label: e.date, value: `${e.event} | Px ${e.priceImpactPct}% Vol ${e.volShiftPct}%`, tone: e.priceImpactPct >= 0 ? 'positive' : 'negative' })), 20, 'EVENT'), provenance: p.events },
    { id: 'docs', title: 'LINKED DOCUMENTS', rows: withMinimumRows((depth?.news.archive ?? []).map((d) => ({ label: `${d.published_at} ${d.source}`, value: d.title, tone: d.title.includes('[SIMULATED]') ? 'accent' : 'neutral' })), 24, 'DOC'), provenance: p.news },
    { id: 'rel', title: 'RELATIONSHIP SUMMARY', rows: withMinimumRows((depth?.relationships?.edges ?? []).map((e) => ({ label: `${e.relationshipType} ${e.weight.toFixed(2)}`, value: `${e.fromId.slice(0, 8)} -> ${e.toId.slice(0, 8)}` })), 18, 'REL'), provenance: p.relationships },
